...
import { defineComponent, useModel, ref, withCtx, createTextVNode, createVNode, toDisplayString } from "vue";
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  props: {
    "isOpenOnMobile": { type: Boolean, ...{ default: false } },
    "isOpenOnMobileModifiers": {}
  },
  emits: ["update:isOpenOnMobile"],
  setup(__props) {
    useSupabaseClient();
    useRouter();
    const route = useRoute();
    const isOpenOnMobile = useModel(__props, "isOpenOnMobile");
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (isOpenOnMobile.value) {
        _push(`<div class="fixed inset-0 bg-black/50 z-30 md:hidden" aria-hidden="true" data-v-1098e5a4></div>`);
      } else {
        _push(`<!---->`);
      }
      _push(`<aside class="${ssrRenderClass([
        ...
        isOpenOnMobile.value ? "translate-x-0 w-64" : "translate-x-full w-64"
        // Slide in/out on mobile
      ])}" aria-label="الشريط الجانبي للإدارة" data-v-1098e5a4><div class="h-16 flex items-center justify-between px-4 bg-gray-900 dark:bg-gray-950 flex-shrink-0" data-v-1098e5a4>`);
      _push(ssrRenderComponent(_component_NuxtLink, {
        to: "/admin",
        class: "text-xl font-bold text-white hover:opacity-80 transition-opacity"
      }, {
      ...
      _push(`<button class="md:hidden text-gray-400 hover:text-white" data-v-1098e5a4><svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" data-v-1098e5a4><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" data-v-1098e5a4></path></svg><span class="sr-only" data-v-1098e5a4>إغلاق القائمة</span></button></div><nav class="flex-1 px-4 py-6 space-y-1 overflow-y-auto custom-scrollbar" data-v-1098e5a4><!--[-->`);
      ssrRenderList(navigation.value, (item) => {
        _push(ssrRenderComponent(_component_NuxtLink, {
          ...
          class: ["flex items-center px-3 py-2.5 rounded-md text-sm font-medium transition-colors duration-150 group", [
            isLinkActive(item.href) ? "bg-primary-600 dark:bg-primary-700 text-white shadow-inner" : "text-gray-300 hover:bg-gray-700/60 dark:hover:bg-gray-700 hover:text-white"
            // تنسيق الرابط غير النشط
          ]]
        }, {
          default: withCtx((_, _push2, _parent2, _scopeId) => {
            if (_push2) {
              _push2(`<span class="${ssrRenderClass([
                ...
                isLinkActive(item.href) ? "text-white" : "text-gray-400 group-hover:text-gray-200"
                // لون الأيقونة يتغير حسب حالة الرابط
              ])}" aria-hidden="true" data-v-1098e5a4${_scopeId}>${item.iconSvg ?? ""}</span><span data-v-1098e5a4${_scopeId}>${ssrInterpolate(item.name)}</span>`);
            } else {
              return [
                createVNode("span", {
      ...
      _push(`<!--]--></nav><div class="px-4 py-4 border-t border-gray-700 dark:border-gray-800 mt-auto flex-shrink-0" data-v-1098e5a4><button class="w-full flex items-center px-3 py-2 rounded-md text-sm font-medium text-red-400 hover:bg-red-900/50 hover:text-red-300 transition-colors duration-150 group" data-v-1098e5a4><span class="w-5 h-5 me-3 flex-shrink-0 text-red-500 group-hover:text-red-400 transition-colors" aria-hidden="true" data-v-1098e5a4>${logoutIcon}</span><span data-v-1098e5a4>تسجيل الخروج</span></button></div></aside><!--]-->`);
    };
  }
});
export {
...
//# sourceMappingURL=AdminSidebar.vue2.mjs.map
