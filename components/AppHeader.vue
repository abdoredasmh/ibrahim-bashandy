  ...
  <header class="bg-white dark:bg-gray-900 shadow-md sticky top-0 z-50"> 
    <nav class="container mx-auto px-4 py-3 flex items-center justify-between">
      ...
      <ul class="hidden md:flex items-center gap-x-6 lg:gap-x-8 text-brown-dark dark:text-gray-300"> 
        <li><NuxtLink to="/" class="hover:text-olive-green dark:hover:text-yellow-400 transition-colors">الرئيسية</NuxtLink></li>
        <li><NuxtLink to="/lessons" class="hover:text-olive-green dark:hover:text-yellow-400 transition-colors">الدروس</NuxtLink></li>
        <li><NuxtLink to="/books" class="hover:text-olive-green dark:hover:text-yellow-400 transition-colors">الكتب</NuxtLink></li>
        <li><NuxtLink to="/study" class="hover:text-olive-green dark:hover:text-yellow-400 transition-colors">الدراسة</NuxtLink></li>
        <li><NuxtLink to="/ask" class="hover:text-olive-green dark:hover:text-yellow-400 transition-colors">اسأل الشيخ</NuxtLink></li>
        <li><NuxtLink to="/about" class="hover:text-olive-green dark:hover:text-yellow-400 transition-colors">عن الشيخ</NuxtLink></li>
       
        <li v-if="isAdmin">
          <NuxtLink to="/admin" class="text-red-600 dark:text-red-400 hover:text-red-800 dark:hover:text-red-300 font-semibold transition-colors">
             لوحة التحكم
          </NuxtLink>
        </li>
      ...
      <div class="flex items-center gap-x-4">
        ...
        <NuxtLink to="/notifications" class="relative text-brown-dark dark:text-gray-300 hover:text-olive-green dark:hover:text-yellow-400 transition-colors" aria-label="الإشعارات"> 
          <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" aria-hidden="true">
        ...
        <ClientOnly> 
          <div v-if="isLoggedIn"> 
            <div class="relative" ref="dropdownRef">
              <button @click="toggleDropdown" class="flex items-center gap-x-2 hover:opacity-80 transition-opacity" aria-expanded="isDropdownOpen.toString()" aria-controls="user-menu">
                <div class="w-8 h-8 rounded-full overflow-hidden bg-olive-green text-white flex items-center justify-center text-sm font-semibold"> 
                  {/* استخدام userAvatar و displayName من storeToRefs */}
              ...
              <transition name="dropdown-fade">
                <div
                 ...
                 >
                   ...
                   <NuxtLink v-if="isAdmin" to="/admin" role="menuitem" tabindex="-1" class="flex items-center gap-2 px-4 py-2 text-sm text-right text-red-600 dark:text-red-400 hover:bg-beige-light dark:hover:bg-gray-700 font-semibold">
                   ...
                   <NuxtLink to="/profile" role="menuitem" tabindex="-1" class="flex items-center gap-2 px-4 py-2 text-sm text-right text-brown-dark dark:text-gray-200 hover:bg-beige-light dark:hover:bg-gray-700">
                      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-4 h-4" aria-hidden="true">
        ...
        <button @click="toggleMobileMenu" class="md:hidden text-brown-dark dark:text-gray-300 hover:text-olive-green dark:hover:text-yellow-400 transition-colors" aria-label="فتح القائمة">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" aria-hidden="true">
  ...
  <transition name="slide-fade">
    ...
    >
      <div class="flex items-center justify-between mb-4">
        ...
        <button @click="closeSidebar" class="text-brown-dark dark:text-gray-300 hover:text-olive-green dark:hover:text-yellow-400" aria-label="إغلاق القائمة">
      ...
      <ul class="space-y-3 text-brown-dark dark:text-gray-300">
        <li><NuxtLink to="/" @click="closeSidebar" class="block hover:text-olive-green dark:hover:text-yellow-400">الرئيسية</NuxtLink></li>
        <li><NuxtLink to="/lessons" @click="closeSidebar" class="block hover:text-olive-green dark:hover:text-yellow-400">الدروس</NuxtLink></li>
        <li><NuxtLink to="/books" @click="closeSidebar" class="block hover:text-olive-green dark:hover:text-yellow-400">الكتب</NuxtLink></li>
        <li><NuxtLink to="/study" @click="closeSidebar" class="block hover:text-olive-green dark:hover:text-yellow-400">الدراسة</NuxtLink></li>
        <li><NuxtLink to="/ask" @click="closeSidebar" class="block hover:text-olive-green dark:hover:text-yellow-400">اسأل الشيخ</NuxtLink></li>
        <li><NuxtLink to="/about" @click="closeSidebar" class="block hover:text-olive-green dark:hover:text-yellow-400">عن الشيخ</NuxtLink></li>
         {/* --- رابط لوحة التحكم للمشرف في القائمة الجانبية --- */}
         <li v-if="isAdmin">
           <NuxtLink to="/admin" @click="closeSidebar" class="block text-red-600 dark:text-red-400 hover:text-red-800 dark:hover:text-red-300 font-semibold">
             لوحة التحكم
           </NuxtLink>
         </li>
         {/* --------------------------------------------- */}
...
import { ref, onMounted, onBeforeUnmount, computed } from 'vue'; // استيراد computed
import { useUserStore } from '~/stores/user';
import { storeToRefs } from 'pinia'; // استيراد storeToRefs
import { useColorMode } from '#imports'; // استيراد useColorMode

const userStore = useUserStore();
// جعل الحالات و getters تفاعلية باستخدام storeToRefs
const { isLoggedIn, displayName, userAvatar, isAdmin } = storeToRefs(userStore);

const isDropdownOpen = ref(false);
const isSidebarOpen = ref(false);
const dropdownRef = ref<HTMLElement | null>(null);
const unreadNotifications = ref(0); // جعلها 0 افتراضياً، يمكن جلبها لاحقًا

// --- حالة الوضع الداكن ---
const isDark = ref(false);
const colorMode = useColorMode();

// تزامن حالة isDark مع colorMode.preference عند التحميل
onMounted(() => {
  // يجب الوصول لـ colorMode.value للحصول على القيمة الحالية (light/dark)
  isDark.value = colorMode.value === 'dark';
  // الاستماع لتغييرات localStorage
   window.addEventListener('storage', handleStorageChange);
   // الاستماع لحدث 'change' من colorMode (قد لا يكون ضرورياً مع مراقبة localStorage)
   // watch(colorMode, (newMode) => { isDark.value = newMode.preference === 'dark'; });
});
...
const toggleTheme = () => {
  // تحديد القيمة الجديدة التي نريد تعيينها
  const newPreference = isDark.value ? 'light' : 'dark';
  colorMode.preference = newPreference; // تحديث التفضيل (سيقوم بتحديث localStorage والكلاس)
  isDark.value = newPreference === 'dark'; // تحديث حالتنا المحلية
}

// دالة لمعالجة التغييرات من تبويبات أخرى
function handleStorageChange(event: StorageEvent) {
  if (event.key === 'nuxt-color-mode') { // اسم المفتاح الافتراضي
    isDark.value = event.newValue === 'dark';
    // تحديث colorMode.value ليتزامن مع localStorage إذا تغير من تبويب آخر
     if (colorMode.value !== event.newValue) {
        colorMode.preference = event.newValue as 'light' | 'dark' | 'system';
     }
  }
}
// -----------------------
...
const handleClickOutside = (e: MouseEvent) => {
  ...
  // لا نحتاج لإغلاق القائمة الجانبية هنا لأن الخلفية تقوم بذلك
}
...
<style scoped>
/* تحسين active class ليشمل الوضع الداكن */
.router-link-exact-active:not(.admin-link) { /* إضافة not(.admin-link) إذا أردت استثناء رابط الأدمن */
  @apply text-olive-green dark:text-yellow-400 font-semibold;
}
/* يمكنك إضافة كلاس مميز لرابط الأدمن إذا أردت تنسيقاً مختلفاً له */
.admin-link.router-link-exact-active {
   @apply text-red-700 dark:text-red-500 font-bold; /* مثال */
}
...
/* Corrected slide direction for RTL */
html[dir="rtl"] .slide-fade-enter-from,
html[dir="rtl"] .slide-fade-leave-to {
   transform: translateX(100%); /* Slide from right for RTL */
}
/* Add fallback for LTR if needed, though default is usually correct */
html:not([dir="rtl"]) .slide-fade-enter-from,
html:not([dir="rtl"]) .slide-fade-leave-to {
   transform: translateX(-100%); /* Slide from left for LTR */
}
</style>