      ...
      <NuxtLink
        ...
        :class="[
          isLinkActive(item.href)
            ? 'bg-indigo-600 text-white shadow-inner' // Active link style
            : 'text-gray-300 hover:bg-gray-700/60 dark:hover:bg-gray-700 hover:text-white' // Inactive link style
        ]"
      >
        <span
          v-html="item.iconSvg"
          :class="[
            ...
            isLinkActive(item.href) ? 'text-white' : 'text-gray-400 group-hover:text-gray-200' // Icon color
          ]"
          aria-hidden="true"
        ></span>
        <span>{{ item.name }}</span>
...
// State for mobile sidebar visibility (controlled by parent layout)
// We use defineModel for two-way binding with the parent
const isOpenOnMobile = defineModel<boolean>('isOpenOnMobile', { default: false });

// Function to close sidebar (emits update to parent)
...
const isLinkActive = (href: string) => {
    ...
    // Starts with for other sections
...
const navigation = ref([
  ...
  { name: 'إدارة الاختبارات', href: '/admin/quizzes', iconSvg: quizzesIcon }, // Added Quizzes
  { name: 'إدارة الفئات', href: '/admin/categories', iconSvg: categoriesIcon },
  { name: 'إدارة التعليقات', href: '/admin/comments', iconSvg: commentsIcon }, // Added Comments
  { name: 'اسأل الشيخ', href: '/admin/ask-sheikh', iconSvg: askSheikhIcon },
  { name: 'إدارة المستخدمين', href: '/admin/users', iconSvg: usersIcon },
  { name: 'إعدادات الموقع', href: '/admin/settings', iconSvg: settingsIcon }, // Added Settings
]);
...
/* Ensure sidebar takes full height in flex column */
aside { display: flex; flex-direction: column; height: 100vh; }

/* Fix for mobile view potentially being pushed down */
