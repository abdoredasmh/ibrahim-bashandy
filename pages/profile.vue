  ...
  <div class="container mx-auto px-4 py-8">
    ...
    <ClientOnly>
      ...
      <div v-if="userStore.profile" class="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <!-- Column 2 & 3: Edit Forms, Courses, Messages -->
        <div class="md:col-span-2 space-y-6">
          ...
          <!-- Private Messages Section with Reply -->
          <BaseCard>
            <template #title>الرسائل الواردة (ردود الشيخ)</template>
            <template #content>
              ...
              <div v-else-if="!privateMessages || privateMessages.length === 0" class="text-gray-500 p-4 text-center">لا توجد رسائل واردة حاليًا.</div>
              <div v-else class="space-y-6"> {/* Increased spacing */}
                <div v-for="(message, index) in privateMessages" :key="message.id"
                  ...
                  <div class="mt-4 pt-4 border-t border-gray-200 dark:border-gray-600">
                    ...
                    <div v-if="message.user_reply_text">
                    ...
                    <!-- Show reply form if no reply exists yet -->
                    <div v-else>
...
import type { Database, Tables, Json } from '~/types/database.types'; // Import Json type if needed
...
// Update PrivateMessage type to include new reply fields
...
const isMarkingRead = ref<number | false>(false); // Track which message is being marked

// --- Reply State ---
const replyingTo = ref<number | null>(null); // ID of the message being replied to
const replyText = ref('');
const isSubmittingReply = ref(false);
const replyError = ref<number | null>(null); // Store message ID if error occurs for specific reply
const genericReplyError = ref<string | null>(null); // General error message for reply submission
...
const { data: privateMessages, pending: pendingMessages, error: errorMessages, refresh: refreshMessages } = useAsyncData<PrivateMessage[]>(
  'privateMessages',
  async () => {
  ...
  try {
    ...
    if(updates.bio === userStore.profile.bio) delete updates.bio;
     if (Object.keys(updates).length <= 2) {
          profileSuccessMessage.value = "لا توجد تغييرات لحفظها.";
          isUpdatingProfile.value = false; return;
     }
    const { error } = await client.from('profiles').upsert(updates);
    if (error) throw error;
    await userStore.fetchProfile();
    profileSuccessMessage.value = 'تم تحديث الملف الشخصي بنجاح!';
    setTimeout(() => profileSuccessMessage.value = null, 3000);
  } catch (err: any) {
    console.error('Error updating profile:', err);
    profileError.value = err.message || 'فشل تحديث الملف الشخصي.';
...
async function handleAvatarUpload(event: Event) {
  ...
  try {
    ...
    if (!urlData?.publicUrl) throw new Error("لم يتم العثور على رابط الصورة.");
    ...
    else if (err.message.includes("mime type")) avatarError.value = 'نوع الملف غير مسموح به.';
...
async function changePassword() {
  ...
  try {
    ...
    setTimeout(() => passwordSuccessMessage.value = null, 3000);
  } catch (err: any) {
    console.error('Error changing password:', err);
    passwordError.value = err.message || 'فشل تغيير كلمة المرور.';
...
async function changeEmail() {
   ...
   try {
     const { error } = await client.auth.updateUser({ email: newEmail.value });
     if (error) throw error;
     emailSuccessMessage.value = "تم إرسال رابط التأكيد إلى بريدك الإلكتروني الجديد.";
     newEmail.value = '';
   } catch (err: any) {
      console.error('Error changing email:', err);
      emailError.value = err.message || 'فشل تغيير البريد الإلكتروني.';
   } finally { isChangingEmail.value = false; }
}

async function markMessageAsRead(messageId: number) {
    if (!user.value || isMarkingRead.value) return;
    isMarkingRead.value = messageId; // Set the ID being marked
    try {
        const { error } = await client
            .from('user_private_messages')
            .update({ is_read: true })
            .match({ id: messageId, user_id: user.value.id });
        if (error) throw error;
        // Optimistic UI update
        const msgIndex = privateMessages.value?.findIndex(m => m.id === messageId);
        if (msgIndex !== undefined && msgIndex !== -1 && privateMessages.value) {
            privateMessages.value[msgIndex].is_read = true;
        }
        await refreshMessages(); // Refresh data in background
    } catch (err: any) {
        console.error('Error marking message as read:', err);
    } finally {
        isMarkingRead.value = false; // Reset after completion/error
    }
}

// --- Reply Functions ---
function showReplyForm(messageId: number) {
    replyingTo.value = messageId;
    replyText.value = ''; // Clear previous text
    replyError.value = null;
    genericReplyError.value = null;
}
...
async function submitReply(messageId: number, messageIndex: number) {
    if (!user.value || !replyText.value.trim()) {
        replyError.value = messageId;
        genericReplyError.value = "الرجاء كتابة نص الرد.";
        return;
    }
    ...
    try {
        const replyData = {
            user_reply_text: replyText.value.trim(),
            user_replied_at: new Date().toISOString(),
            admin_read_reply: false // Mark as unread for admin
        };
        ...
        // Update the local message state optimistically or with returned data
        if (privateMessages.value && updatedMessage) {
             privateMessages.value[messageIndex] = { ...privateMessages.value[messageIndex], ...updatedMessage };
        } else {
             // If optimistic update failed or data not returned, refresh all
        ...
        replyError.value = messageId; // Show error under the specific message form
        genericReplyError.value = err.message || 'فشل إرسال الرد.';
    } finally {
        isSubmittingReply.value = false;
    }
}

async function logout() { await userStore.logout(); }
